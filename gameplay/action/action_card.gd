...
func _init(p_id: String = "", p_name: String = "", p_kind: String = BOLT,
		p_power: float = 10.0, p_cooldown: float = 2.0, p_color: Color = Color.WHITE) -> void:
	id = p_id
	display_name = p_name
	kind = p_kind
	power = p_power
...
static func default_hand() -> Array:
	return [
		ActionCard.new("wailing_bolt", "Wailing Bolt", BOLT, 18.0, 1.6, Color(0.55, 0.85, 1.0)),
		ActionCard.new("spirit_lash", "Spirit Lash", LASH, 26.0, 2.6, Color(0.78, 0.96, 1.0)),
		ActionCard.new("drowned_ward", "Drowned Ward", WARD, 34.0, 4.5, Color(0.52, 0.80, 0.72)),
		ActionCard.new("rip_tide", "Rip Tide", RIPTIDE, 24.0, 3.4, Color(0.92, 0.72, 0.42)),
	]
